...
const STATUS_STYLES: Record<string, string> = {
  ...
  expiring: "bg-orange-150 text-orange-700 border-orange-200",
...
function statusLabel(c: Customer): string {
  if (c.subscriptionType === "session" || c.status === "session") return "حصة";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-xl p-6 w-full max-w-sm border border-gray-100 transition-all">
        ...
        <p className="text-sm text-gray-600 mb-6">
          هل أنت متأكد من حذف اشتراك <span className="font-semibold text-gray-900">{name}</span>؟
          <br />
...
function RenewModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={onCancel} />
      <div className="relative bg-white rounded-2xl shadow-xl p-6 w-full max-w-sm border border-gray-100 transition-all max-h-[90vh] overflow-y-auto">
        <h3 className="text-lg font-bold text-gray-900 mb-1">تجديد الاشتراك</h3>
        <p className="text-sm font-bold text-gray-500 mb-5">{customer.name}</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full px-3.5 py-3 rounded-xl border border-gray-300 bg-white text-gray-900 text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent transition"
            />
          </div>

          <div>
            <label className="block text-sm font-bold text-gray-700 mb-1.5">المدة الجديدة (أيام)</label>
            <input
              required
              type="number"
              min="1"
          ...
          {newEndDate && (
            <div className="flex items-center gap-2 px-3.5 py-3 bg-green-50 border border-green-200 rounded-xl text-sm text-green-700 font-semibold">
          ...
          <div className="flex gap-3 pt-2">
            <button
              type="submit"
              disabled={saving}
              className="flex-1 py-3 rounded-xl bg-green-600 text-white text-sm font-bold hover:bg-green-700 active:scale-95 disabled:opacity-60 disabled:cursor-not-allowed transition"
            >
              {saving ? "جارٍ التجديد…" : "تجديد"}
            </button>
            <button
              type="button"
              onClick={onCancel}
              className="flex-1 py-3 rounded-xl bg-gray-100 text-gray-700 text-sm font-bold hover:bg-gray-200 active:scale-95 transition"
            >
              إلغاء
            </button>
          </div>
...
function CustomersContent() {
  ...
  return (
    <div className="min-h-full bg-gray-50/50 pb-24 sm:pb-8">
      ...
      {successMsg && (
        <div className="fixed bottom-20 sm:bottom-6 left-1/2 -translate-x-1/2 z-50 px-5 py-3.5 bg-green-600 text-white text-sm font-bold rounded-xl shadow-lg transition-all">
          {successMsg}
        </div>
      )}
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          <div className="flex items-center gap-3">
            <h2 className="text-xl sm:text-2xl font-black text-gray-900">الاشتراكات</h2>
            {urlFilter && FILTER_LABELS[urlFilter] && (
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-blue-50 text-blue-700 border border-blue-100">
                ...
                <Link href="/customers" className="hover:text-blue-900 font-black">✕</Link>
              </span>
            )}
          </div>
          <div>
            <Link
              href="/customers/new"
              className="inline-flex items-center justify-center gap-2 w-full sm:w-auto px-5 py-3 rounded-xl bg-blue-600 text-white text-sm font-bold hover:bg-blue-700 transition active:scale-[0.98] shadow-md shadow-blue-200"
            >
          ...
          <>
            ...
            <div className="sm:hidden space-y-4">
              {filtered.map((c) => {
                ...
                return (
                  <div key={c.id} className="bg-white rounded-2xl border border-gray-200 p-4 shadow-sm space-y-3.5">
                    {/* Header: Name and Status Badge */}
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <h4 className="font-black text-gray-900 text-base leading-snug">{c.name}</h4>
                        <p className="text-xs text-gray-400 font-bold mt-0.5">{c.phone}</p>
                      </div>
                      <span className={`inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-black border ${STATUS_STYLES[liveStatus]}`}>
                        {statusLabel(c)}
                      </span>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 text-xs bg-gray-50/50 rounded-xl p-3 border border-gray-100">
                      {c.subscriptionType === "session" || c.status === "session" ? (
                        <div className="col-span-2">
                          <span className="text-gray-400 font-bold block mb-0.5">تاريخ التسجيل</span>
                          <span className="text-gray-700 font-black">{c.startDate || "—"}</span>
                        </div>
                      ) : (
                        <>
                          <div>
                            <span className="text-gray-400 font-bold block mb-0.5">تاريخ البدء</span>
                            <span className="text-gray-700 font-black">{c.startDate}</span>
                          </div>
                          <div>
                            <span className="text-gray-400 font-bold block mb-0.5">تاريخ الانتهاء</span>
                            <span className="text-gray-700 font-black">{c.endDate}</span>
                          </div>
                        </>
                      )}
                      <div className="col-span-2 pt-1.5 border-t border-gray-100">
                        <span className="text-gray-400 font-bold">قيمة الاشتراك: </span>
                        <span className="text-gray-800 font-black text-sm">{c.price.toLocaleString()} جنيه</span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 pt-1">
                      <a
                        ...
                        className="flex-1 py-3 rounded-xl bg-green-50 text-green-700 font-bold text-xs flex items-center justify-center gap-1.5 border border-green-100 active:scale-95 transition"
                      >
                      ...
                      {c.subscriptionType !== "session" && c.status !== "session" && (
                        <button
                          ...
                          className="flex-1 py-3 rounded-xl bg-blue-50 text-blue-700 font-bold text-xs flex items-center justify-center gap-1 border border-blue-100 active:scale-95 transition"
                        >
                          تجديد
                        </button>
                      )}
                      ...
                      <Link
                        ...
                        className="p-3 rounded-xl bg-gray-50 border border-gray-150 text-gray-600 active:scale-95 transition"
                        title="تعديل"
                      >
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <table className="min-w-full text-sm">
                  <thead>
                    <tr className="bg-gray-50 border-b border-gray-200">
                      <th className="text-right px-5 py-4.5 font-bold text-gray-600">الاسم</th>
                      <th className="text-right px-5 py-4.5 font-bold text-gray-600">الهاتف</th>
                      <th className="text-right px-5 py-4.5 font-bold text-gray-600">البداية</th>
                      <th className="text-right px-5 py-4.5 font-bold text-gray-600">النهاية</th>
                      <th className="text-right px-5 py-4.5 font-bold text-gray-600">السعر</th>
                      <th className="text-right px-5 py-4.5 font-bold text-gray-600">الحالة</th>
                    ...
                    {filtered.map((c) => {
                      ...
                      return (
                        <tr key={c.id} className="hover:bg-gray-50/50 transition">
                          ...
                          <td className="px-5 py-4 text-gray-600 font-semibold">{c.phone}</td>
                          <td className="px-5 py-4 text-gray-600 font-semibold">{c.startDate || "—"}</td>
                          <td className="px-5 py-4 text-gray-600 font-semibold">{isSession ? "—" : c.endDate}</td>
                          <td className="px-5 py-4 text-gray-700 font-bold">{c.price.toLocaleString()} جنيه</td>
                          <td className="px-5 py-4">
                            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold border ${STATUS_STYLES[liveStatus]}`}>
                              {statusLabel(c)}
                            </span>
                          </td>
                          <td className="px-5 py-4">
...
export default function CustomersPage() {
  return (
    <Suspense fallback={
      <div className="min-h-full bg-gray-50 pb-24 flex items-center justify-center">
